...
def load_and_filter_events(args):
    
    base_path=f"{DATA_BASE_PATH}/data/{args.data_name}/events_soho"
...
def data_loader(args):
    ...
    df = domain_info.get_time_series(args.ticker , data_save_base = DATA_BASE_PATH ,\
                                      hori='soho' ,  data_name=args.data_name)
...
def event_retrieval(prompt_params  , event_list , model_name , ticker , is_debug = 0 ):
    ...
    sector, target, memory_list, missing_list , memory_cap , missing_cap =\
                    memory_tool.get_soho_stock_memorys(ticker , memory_dir = MEMORY_DIR)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--repeat", type=str, default='', choices=['' , 'v0' , 'v1' , 'eval'] , 
                                 help="This is to help replicate the experiments.")
    parser.add_argument("-n", "--data_name", type=str, default='stock', help="prediction Data")
...
---- Lookback Window: 14.   Prediction Horizon: 1 ----

Avg Rank | Model/Baseline          | Avg Accuracy (%)
-------------------------------------------------------
5.2      | Gemini (All Evts, 2e)   | 41.07           
5.7      | Gemini (All Evts, 5e)   | 39.33           
6.1      | Claude-sonnet-4p6 (All Evts, 2e) | 39.07           
6.5      | Claude-sonnet-4p6 (No Evts) | 36.40           
6.7      | Claude-opus-4p6 (All Evts, 2e) | 36.67           
6.8      | Gemini-flash-3p1 (All Evts, 2e) | 39.20           
6.8      | Gemini-flash-3p1 (All Evts, 5e) | 38.80           
6.9      | Claude-opus-4p6 (All Evts, 5e) | 37.87           
7.4      | Claude-sonnet-4p6 (All Evts, 5e) | 38.27           
7.8      | Claude-opus-4p6 (No Evts) | 33.87           
8.1      | Gemini-flash-3p1 (No Evts) | 34.40           
8.5      | Gemini (No Evts)        | 34.93           
8.5      | Baseline                | 33.33           
...
Avg Rank | Model/Baseline          | Avg Accuracy (%)
-------------------------------------------------------
5.5      | Gemini (All Evts, 5e)   | 28.00           
5.5      | Claude-sonnet-4p6 (No Evts) | 27.47           
5.6      | Claude-opus-4p6 (All Evts, 2e) | 27.47           
5.8      | Claude-opus-4p6 (No Evts) | 26.40           
6.7      | Claude-opus-4p6 (All Evts, 5e) | 27.60           
6.8      | Gemini-flash-3p1 (No Evts) | 25.87           
7.1      | Gemini (All Evts, 2e)   | 26.27           
7.2      | Gemini-flash-3p1 (All Evts, 5e) | 26.13           
7.5      | Claude-sonnet-4p6 (All Evts, 2e) | 25.33           
7.5      | Gemini-flash-3p1 (All Evts, 2e) | 26.93           
8.3      | Gemini (No Evts)        | 24.27           
8.5      | Claude-sonnet-4p6 (All Evts, 5e) | 24.80           
9.2      | Baseline                | 20.00 
