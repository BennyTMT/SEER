...
def run_search_stock_parallel(model_name, ticker, start_date, end_date, 
    ...
    # multi_days_taxi_events_search
    if 'taxi' in agent_domain:
        event_save_path = f'{DATA_BASE_PATH}/data/taxi/events/{ticker}'
    elif 'elec' in agent_domain:
        event_save_path = f'{DATA_BASE_PATH}/data/elec/events/{ticker}'
    else:
        raise ValueError(f"DO NOT Support {agent_domain}!")
    ...
    #     print(f"ERROR: Failed to fetch domain info: {ticker}")
    #     print(f'Using Ticket Name as the Full Name')
